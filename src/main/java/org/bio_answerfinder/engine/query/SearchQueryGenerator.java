...
public class SearchQueryGenerator {
    ...
    void findAcronyms(List<SpanPOS> spList, List<QueryPart> queryParts) {
        ...
        for (int i = 0; i < upper_limit; i++) {
            ...
            if (theAcronym != null) {
                queryParts.add(new QueryPart(new SearchTerm(theAcronym, false, "ACR")));
            }
        }
    }
        ...
        for (SearchTerm st : qp.getSearchTerms()) {
            ...
            if (!posList.isEmpty()) {
                ...
                if (!lastSP.getLemma().equals(lastSP.getToken())) {
                    ...
                    boolean isPhrase = sb.toString().indexOf(' ') != -1;
                    SearchTerm lemmaST = new SearchTerm(sb.toString(), isPhrase,
                            isPhrase ? "NP" : posList.get(0).getPosTag());
    ...
    public static void expandByVocabulary(List<QueryPart> queryParts, TObjectFloatHashMap<String> vocabulary) {
        ...
        for (String newTerm : newTerms) {
            ...
            if (phrase) {
                queryParts.add(new QueryPart(new SearchTerm(newTerm, true, "NP")));
            } else {
                queryParts.add(token2QueryPart(newTerm, "NEXP")); // for noun vocabulary expansions
            }
        }
    }
        ...
        */
        ...
        for (Iterator<List<SpanPOS>> it = subPhrases.iterator(); it.hasNext(); ) {
            ...
            if (subPhrase.size() == 1) {
                if (qp == null) {
                    qp = new QueryPart();
                    qp.addSearchTerm(new SearchTerm(subPhrase.get(0).getToken(), false,
                            subPhrase.get(0).getPosTag()), toConnective(cc));
                    queryParts.add(qp);
                } else {
                    qp.addSearchTerm(new SearchTerm(subPhrase.get(0).getToken(),
                            false, subPhrase.get(0).getPosTag()), toConnective(cc));
                }
            } else {
                ...
                SearchTerm st = new SearchTerm(phrase, phrase.indexOf(' ') != -1, "NP");
                if (qp == null) {
                    qp = new QueryPart();
                    queryParts.add(qp);
                }
    ...
    List<QueryPart> handleAppositive(DependencyNode apposNode, List<QueryPart> queryParts, List<Acronym> acronymList) {
        ...
        if (apposNode == null || !acronymList.isEmpty()) {
            ...
            QueryPart qp = new QueryPart(new SearchTerm(acr.getExpansion(0).getExpansion(), true, "AE"));
            qp.addSearchTerm(new SearchTerm(acr.getAcronym(), false, "ACR"), Connective.OR);
            queryParts.add(qp);
        ...
        if (longestMatch != null) {
            ...
            if (longestMatch.matches(apposNode.getToken())) {
                qp = new QueryPart(new SearchTerm(apposNode.getToken(), false, apposNode.getPosTag()));
                if (!apposNode.getToken().equalsIgnoreCase(apposNode.getLemma())) {
                    SearchTerm st = new SearchTerm(apposNode.getLemma(), false, apposNode.getPosTag());
                    qp.addSearchTerm(st, Connective.OR);
                } else {
                }
            } else {
                qp = conditionAppositiveExp(longestMatch, apposNode.getToken());
                qp.addSearchTerm(new SearchTerm(apposNode.getToken(), false, apposNode.getPosTag()), Connective.OR);
                if (!apposNode.getToken().equalsIgnoreCase(apposNode.getLemma())) {
                    qp.addSearchTerm(new SearchTerm(apposNode.getLemma(), false, apposNode.getPosTag()), Connective.OR);
                }
            }
            queryParts.add(qp);
        }
        ...
        if (qp.contains(apposToken)) {
            ...
            if (matcher.find()) {
                if (matcher.group(1).equals(apposToken)) {
                    ...
                    boolean isPhrase = term.indexOf(' ') != -1;
                    QueryPart qp1 = new QueryPart(new SearchTerm(term, isPhrase, isPhrase ? "NP" : "N"));
                    return qp1;
                }
            }
        }
     ...
     * @return
     */
    public static QueryPart token2QueryPart(String token, String posTag) {
        if (isSlashWord(token)) {
            ...
            if (token2.length() == 1 && NumberUtils.isNumber(token2) &&
                ...
                qp.addSearchTerm(new SearchTerm(token1, false, posTag), Connective.OR);
                qp.addSearchTerm(new SearchTerm(token1.substring(0, token1.length() - 1) + token2,
                        false, posTag), Connective.NONE);

            } else {
                if (token2.length() == 1 && NumberUtils.isNumber(token2)) {
                    qp.addSearchTerm(new SearchTerm(token1, false, posTag), Connective.NONE);
                } else {
                    qp.addSearchTerm(new SearchTerm(token1, false, posTag), Connective.OR);
                    qp.addSearchTerm(new SearchTerm(token2, false, posTag), Connective.NONE);
                }
            }
            return qp;
/*
        ...
        */
            ...
            return new QueryPart(new SearchTerm(token, false, posTag));
        }
    }
