...
class Sentence2QueryHandler {
    ...
    public Sentence2QueryHandler invoke() throws ParseTreeManagerException {
        ...
        for (DataRecord.Chunk chunk : chunks) {
            if (chunk.getType().equals("NP")) {
                ...
                if (np.indexOf(" ") != -1) {
                    SearchQuery.SearchTerm st = new SearchQuery.SearchTerm(np, true);
                    queryParts.add(new SearchQuery.QueryPart(st));
                } else {
                    // handle terms containing / and -
                    queryParts.add(SearchQueryGenerator.token2QueryPart(np));
        ...
        for (SpanPOS sp : nounSPList) {
            if (!seenNounSPSet.contains(sp)) {
                queryParts.add(new SearchQuery.QueryPart(new SearchQuery.SearchTerm(sp.getToken(), false)));
            }
        }
                ...
                if (!nominalizations.isEmpty()) {
                    ...
                    if (searchQueryGenerator.verbUtils != null) {
                        ...
                        for (Iterator<String> it = verbConjugations.iterator(); it.hasNext(); ) {
                            String vc = it.next();
                            qp.addSearchTerm(new SearchQuery.SearchTerm(vc, false), SearchQuery.Connective.OR);
                        }
                    } else {
                        qp.addSearchTerm(new SearchQuery.SearchTerm(verbLemma, false), SearchQuery.Connective.OR);
                        qp.addSearchTerm(new SearchQuery.SearchTerm(verbLemmaList.get(verbLemma), false), SearchQuery.Connective.OR);
                    }
                    for (Iterator<String> it = nominalizations.iterator(); it.hasNext(); ) {
                        String nom = it.next();
                        qp.addSearchTerm(new SearchQuery.SearchTerm(nom, false), it.hasNext() ? SearchQuery.Connective.OR : SearchQuery.Connective.NONE);
                    }
                    queryParts.add(qp);
                } else {
                    if (searchQueryGenerator.verbUtils != null) {
                        ...
                        queryParts.add(new SearchQuery.QueryPart(new SearchQuery.SearchTerm(verbLemma, false)));
                    }
    ...
    void addVerbConjugations(String verbLemma) {
        ...
        for (Iterator<String> it = verbConjugations.iterator(); it.hasNext(); ) {
            String vc = it.next();
            qp.addSearchTerm(new SearchQuery.SearchTerm(vc, false), it.hasNext() ? SearchQuery.Connective.OR : SearchQuery.Connective.NONE);
        }
        queryParts.add(qp);
    }
}
