...
"""BERT finetuning runner.
   Modified for BioASQ  question answer classification task (IBO).
"""
...
flags.DEFINE_bool(
    "do_export", False,
    "Whether to export the model for Tensorflow Serving.")
...
class BioAsqProcessor(DataProcessor):
    """Processor for the bioasq QA engine answer selection data set."""
    ...
    def get_test_examples(self, data_dir):
        """Uses dev examples since there is no test case."""
        return self._create_examples(
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (prediction, example) in zip(result, predict_examples):
                ...
                output_line += "\t" + example.guid + "\t" + str(example.label) + "\n"
